...
abstract class StockSubInfoUI<T> : ViewBinderComponent<T> {
    fun ViewGroup.cell(ankoContext: AnkoContext<T>, title: String, key: String, converter: OneWayConverter<*, CharSequence> = EmptyOneWayConverter1()): View = with(ankoContext) {
...
class StockSubInfoHSUI<T> : StockSubInfoUI<T>() {
...
class StockSubInfoHKUI<T> : StockSubInfoUI<T>() {
    override fun builder(): AnkoContext<T>.() -> Unit = {
        val ankoContext = this
        gridLayout {
            cell(ankoContext, "最高", "high").lparams(GridLayout.spec(0, GridLayout.TOP), GridLayout.spec(0, GridLayout.FILL, 1f)) { rightMargin = dip(5)}
...
class StockInfoUI<T> : ViewBinderComponent<T> {
    val visibility4HS = OneWayConverter<Any?, Boolean> { true }

    val viewOfMarket = OneWayConverter<MarketType, ViewBinderComponent<T>> { source -> if (source == MarketType.SH_SZ) StockSubInfoHSUI() else StockSubInfoHKUI() }
